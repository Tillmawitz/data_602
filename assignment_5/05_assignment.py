...
def exercise11(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    array_1d = np.arange(n, dtype=int)
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.array([[(i + j + 1) % 2 for j in range(2 * n)] for i in range(2 * n)])
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(data = np.cumsum(np.random.randint(0, n, n)), index = pd.date_range(start='1/1/2010', periods=n))
    s.plot()
    # ------ Place code above here /\ /\ /\ ------ 
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame(pd.Series(words).map(lambda x: len(x)).value_counts())
